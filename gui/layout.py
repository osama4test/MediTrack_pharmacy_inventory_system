...
def build_gui():
    ...
    # Changed Mfg Date → Shelf
    labels = ["Name*", "Batch No", "Shelf*", "Expiry Date (YYYY-MM-DD)*", "Quantity*", "Price", "Demand"]
    entries = []
    ...
    # Navigation + Delete/Export
    nav_frame = tb.Frame(root)
    nav_frame.pack(fill="x", padx=15)

    buttons_info = [
        ("💳 Checkout", "primary outline", lambda: open_checkout_window(on_checkout_complete=events.load_data)),
        ("↩️ Return Medicine", "warning outline", lambda: open_return_window(on_return_complete=events.load_data)),
        ("📅 View Sales Report", "info outline", open_sales_report_window),
        ("🗑️ Delete Selected", "danger outline", events.delete_selected),
        ("📤 Export to CSV", "success outline", events.export_to_csv),
    ]

    for text, style, cmd in buttons_info:
        btn = tb.Button(nav_frame, text=text, bootstyle=style, width=20, command=cmd)
        btn.pack(side="left", padx=10, pady=5)
        if "Delete" in text:
            btn_delete = btn
        if "Export" in text:
            btn_export = btn
        create_tooltip(btn, text)

    # Tree Table
    tree_frame = tb.Frame(root)
    tree_frame.pack(fill="both", expand=True, padx=15, pady=10)

    columns = ["Name", "Batch", "Shelf", "Expiry Date", "Quantity", "Price", "Demand", "Status"]
